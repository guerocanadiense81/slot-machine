...
// Global toggles (for testing win conditions)
...
function checkWin() {
  ...
  for (let rowIndex = 0; rowIndex < 3; rowIndex++) {
    ...
    reels.forEach((reel, reelIndex) => {
      ...
      console.log(`Reel ${reelIndex}:`, reel); // Log entire reel
      ...
      if (icons && icons.length > rowIndex) {
        ...
        if (!img) {
          console.warn(`Reel ${reelIndex}, row ${rowIndex} image is undefined.`);
          rowSymbols.push("undefined");
        } else {
          const src = img.getAttribute("src");
          console.log(`Reel ${reelIndex}, row ${rowIndex} src: ${src}`);
          if (!src) {
            console.warn(`Reel ${reelIndex}, row ${rowIndex} has no src.`);
            rowSymbols.push("undefined");
          } else {
    ...
    if (!pause3InRow && rowSymbols.length >= 3) {
      ...
      if (!threeFound) console.log(`Row ${rowIndex}: No 3-in-a-row win detected.`);
    }
    
    // 2-in-a-row win on middle row (rowIndex 1) - count every pair
    if (!pause2InRow && rowIndex === 1 && rowSymbols.length >= 2) {
      ...
      else console.log(`Row 1: Detected ${pairCount} pairs.`);
    }
  }
...
window.spin = function(elem) {
  const betInput = document.getElementById("bet-input");
  const betAmount = betInput && !isNaN(parseFloat(betInput.value)) ? parseFloat(betInput.value) : 50;
  // Total available = deposit + net play balance.
  ...
  // Deduct bet from net play balance.
