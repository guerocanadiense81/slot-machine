...
// Global toggles for win conditions (set to true to disable a given condition)
...
function checkWin() {
  ...
  // Process each row (rows 0, 1, 2)
  for (let rowIndex = 0; rowIndex < 3; rowIndex++) {
    let rowSymbols = [];
    
    reels.forEach((reel, reelIndex) => {
      // Try selecting images within .icon
    ...
    // 2-in-a-row win on middle row (rowIndex 1): Count every distinct consecutive pair.
  ...
  if (totalMultiplier > 0) {
    ...
    console.log(`Winnings calculated: ${winnings} MET (Bet: ${betAmount} x Multiplier: ${totalMultiplier}).`);
